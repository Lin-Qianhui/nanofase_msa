...
program main
    ...
    if (modelConfig%runToSteadyState .and. trim(modelConfig%steadyStateMode) == 'sediment_size_distribution') then
        call output%initSedimentSizeDistribution()
        call LOGR%add("Running in steady state mode " // trim(modelConfig%steadyStateMode), COLOR_BLUE)
    end if
    
    ! Set up checkpointing and check if we're meant to be reinstating a checkpoint now
    call checkpt%init(env, modelConfig%checkpointFile)
    if (modelConfig%reinstateCheckpoint) then
        call checkpt%reinstate(preserve_timestep=modelConfig%preserveTimestep)
    end if

    ! Check if we've been asked to run a warm up period, which runs the first N timesteps' worth
    ! of data, excluding NM inputs, through the model, where N is specified by modelConfig%warmUpPeriod
    if (modelConfig%warmUpPeriod > 0) then
        ! Log some info about it
        call LOGR%add("Running for warm up period of " // trim(str(modelConfig%warmUpPeriod)) // " time steps", COLOR_BLUE)
        ! Run the model with the warmUp flag
        do t = 1, modelConfig%warmUpPeriod
        ...
        if (modelConfig%saveCheckpointAfterWarmUp) then
            call checkpt%save(0)
        end if
    end if
    ...
    do while (.not. steadyStateReached)
        ...
        if (modelConfig%runToSteadyState) then
        ...
        do k = 1, modelConfig%nChunks
            ...
            call output%finaliseChunk(tPreviousChunk + 1, k == modelConfig%nChunks)
        ...
        if (modelConfig%runToSteadyState) then
            ...
            if (trim(modelConfig%steadyStateMode) == 'sediment_size_distribution') then
                ...
                if (delta_max <= modelConfig%steadyStateDelta) then
    ...
    if (modelConfig%saveCheckpoint) then
